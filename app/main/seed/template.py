...
def seed_templates():
    records = [
      ...
      { 'title': 'Payment Reminder', 'action': 'PAYMENT_REMINDER', 'path': 'pending_payment_notice.html', 'editable': False, 'desc': 'Send five days prior to the payment date of the client preferably around 12pm', 'subject': 'Your upcoming payment to Elite DMS'},
      ...
      { 'title': 'Cancellation Request (Email)', 'action': 'CANCELLATION_REQUEST', 'path': 'cancellation_request.html', 'editable': False, 'desc': 'Send to Client when status is changed to Request Cancellation action.', 'medium':'EMAIL_SMS', 'subject': 'A request to cancel the Elite DMS program has been received'},
      # Send when EPPS returns status as unsuccessful (not sure what the exact EPPS words are) on any payment.
      { 'title': 'NSF Draft Issue', 'action': 'NSF_DRAFT_ISSUE', 'path': 'nsf_draft_issue.html', 'editable': False, 'desc': 'Send when EPPS returns status as unsuccessful on any payment', 'subject': 'Payment Issue-Please contact your account manager'},
      # Send one hour prior to the clients next scheduled appointment in the CRM.  Ensure the CRM corrects properly for timezones
      { 'title': '1 Hour Appointment Reminder (Email)', 'action': 'HOUR1_APPOINTMENT_REMINDER', 'path': 'h1_appointment_reminder.html', 'editable': False, 'desc': 'Send one hour prior to the clients next scheduled appointment in the CRM.', 'subject': 'Appointment reminder in 1 hour for Elite DMS'},
      # Send when a service user or service manager successfully completes the change payment date process on the EDMS tab
      { 'title': 'Change of Payment Date (Email)', 'action': 'CHANGE_PAYMENT_DATE', 'path': 'change_payment_date.html', 'editable': False, 'date': 'Send when a service user or service manager successfully completes the change payment date.', 'medium':'EMAIL_SMS', 'subject': 'Payment Date Change confirmation'},
      # Send 1 day prior to the client's next scheduled appointment…preferably in the afternoon
      { 'title': '1 Day Appointment Reminder (Email)', 'action': 'DAY1_APPOINTMENT_REMINDER', 'path': 'd1_appointment_reminder.html', 'editable': False, 'desc': 'Send 1 day prior to the clients next scheduled appointment…preferably in the afternoon', 'medium': 'EMAIL_SMS', 'subject': 'Appointment reminder tomorrow for Elite DMS'},
      # Send when the user changes the status to "Complete" after any phone appointment in the CS schedule
      ...
      { 'title': 'NOIR Auto (Email)', 'action': 'NOIR_SENT_ACK', 'path': 'noir_sent_ack.html', 'editable': False, 'desc': 'When specific Debt Status Changes to NOIR Sent (Debt status is on debt creditor tab)', 'subject': 'Update on one of your accounts'},
      ...
      { 'title': 'NOIR EDMS Notice', 'action': 'NOIR_NOTICE', 'path': 'noir_common.html', 'medium': 'FAX', 'editable': False, 'desc': 'Send when Changing status to NOIR Sent, send through fax if there IS a FAX number in the 3rd party debt collector fax number box else POST.'},
      # 
      { 'title': 'NOIR2 EDMS Notice', 'action': 'NOIR_2_NOTICE', 'path': 'noir_2_notice.html', 'medium': 'FAX', 'editable': False, 'desc': 'Send when Changing status to NOIR2 Sent, send through fax if there IS a FAX number in the 3rd party debt collector fax number box else POST.'},
      ...
      { 'title': 'Non Response EDMS Notice', 'action': 'NON_RESPONSE_NOTICE', 'path': 'non_response_notice.html', 'medium': 'FAX', 'editable': False, 'desc': 'Send when changing status to Non-Response Sent automatically after 50 days from the date the P1 dispute letter is sent'},
      ...
      { 'title': 'NOIR FDCPA EDMS', 'action': 'NOIR_FDCPA_NOTICE', 'path': 'fdcpa_insufficient_response.html', 'medium': 'FAX', 'editable': False, 'desc': 'Send when Changing status to NOIR FDCPA Only, and there IS a FAX number in the 3rd party debt collector fax number box.'},
      ...
      { 'title': 'Initial Dispute Sent Notice (Email)', 'action': 'INITIAL_DISPUTE_SENT_ACK', 'path': 'initial_dispute_sent_ack.html', 'editable': False, 'desc': 'Email sent to client when Service user changes the Debt Status to Initial Dispute Sent on a document type of collection letter when sending the P1 dispute letter.', 'subject': 'Dispute Process initiated on one of your accounts'},
      # When Service user or doc processor changes Debt Status to: NOIR 2 Sent, NOIR FDCPA Sent, Sold Package Sent 
      { 'title': 'Other Dispute Sent Notice (Email)', 'action': 'OTHER_DISPUTE_SENT_ACK', 'path': 'other_dispute_sent_ack.html', 'editable': False, 'desc': 'When Service user or doc processor changes Debt Status to: NOIR 2 Sent, NOIR FDCPA Sent, Sold Package Sent', 'subject': 'Dispute process update on one of your accounts'},
      ...
      { 'title': 'Fully Disputed Notice (Email)', 'action': 'FULLY_DISPUTED_NOTICE', 'path': 'fully_disputed_notice.html', 'editable': False, 'desc': 'When Service user or doc processor changes Debt Status to any status that has Fully Disputed in it (Full disputed-non response expired, Full disputed: NOIR 2 expired, Full disputed:NOIR FDCPA Expired, Full Disputed: Manual, Fully disputed)', 'subject': 'Dispute process final on one of your accounts'},
      # When Service user or doc processor changes Debt Status to "Non-Response Sent"
      { 'title': 'Non Response sent notice (Email)', 'action': 'NON_RESPONSE_SENT_ACK', 'path': 'non_response_sent_ack.html', 'editable': False, 'desc': 'When Service user or doc processor changes Debt Status to Non-Response Sent', 'subject': 'Dispute process update on one of your accounts'},

      # When Service user or doc processor clicks on Green Send CP login button on the main tab. 
      { 'title': 'Client Portal Login EDMS (Email)', 'action': 'CLIENT_PORTAL_LOGIN', 'path': 'client_portal_login.html', 'editable': False, 'desc': 'When Service user or doc processor clicks on Green Send CP login button on the main tab.', 'subject': 'Client Portal Information'},
      ...
      { 'title': 'Initial Dispute Mail', 'action': 'INITIAL_DISPUTE_MAIL', 'path': 'initial_dispute_mail.html', 'editable': False, 'medium': 'FAX', 'desc': 'Send FAX when Changing status to Initial Dispute Sent'},
      ...
      { 'title': 'EDMS_3 Day Text (SMS)', 'action': 'DAY3_REMINDER', 'path': 'day3_reminder.txt', 'medium': 'SMS',  'editable': False, 'desc': 'Send 3 days after the client enrolls in the program preferably around 12-1PM'},
      # Payment reminder SMS
      { 'title': 'Notification EPPS Payment(SMS)', 'action': 'SMS_PAYMENT_REMINDER', 'path': 'payment_reminder.txt', 'medium': 'SMS',  'editable': False, 'desc': 'Send SMS Text Message 5 days and 1 day prior to clients projected draft date to notify client of upcoming  EPPS draft payment.'},
    ]
    ...
    for record in records:
        ...
        if tmpl is None:
            ...
            # updated editable field
            editable = False
            if 'editable' in record:
                editable = record['editable']
            tmpl.is_editable = editable 
